...
const TownerOpenData = () => {
  ...
  const [formData, setFormData] = useState<StatData>({
    ...
    ongoingTrips: "89",
    totalDrivers: "892",
    completedTrips: "14,892",
    citiesCovered: "6",
    driverEarnings: "4,567,890",
    ...
    // City Statistics
    cityOne: "Mumbai",
    cityOneTotalTrip: "5,678",
    cityOneActiveDrivers: "234",
    cityOneCompletedTrips: "5,445",

    cityTwo: "Delhi",
    cityTwoTotalTrip: "4,567",
    cityTwoActiveDrivers: "189",
    cityTwoCompletedTrips: "4,378",

    cityThree: "Bangalore",
    cityThreeTotalTrip: "3,456",
    cityThreeActiveDrivers: "156",
    cityThreeCompletedTrips: "3,300",

    // Today's Report
    finishedTrips: "567",
    todayOngoingTrips: "45",
    hailTrips: "234",
    nonResponseTrips: "23",

    // Achievements
    tripsCompleted: "25,678",
    achievementTotalDrivers: "1,234",
    platformUptime: "99.9%",
  ...
  const fetchTownerData = async () => {
    try {
      const response = await fetch("/api/admin/viewTowData");
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch("/api/admin/updateTowData", {
        method: "POST",
        headers: {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <StatCard
          ...
          stats={[
            ...
            { label: "Micro-Entrepreneurs", key: "totalDrivers" },
            { label: "Completed Trips", key: "completedTrips" },
            // { label: "Cities Covered", key: "citiesCovered" },
            { label: "Driver Earnings", key: "driverEarnings" },
            // { label: "Registration Count", key: "registrationCount" },
        ...
        <StatCard
          ...
          isEditing={isEditing}
          onInputChange={handleInputChange}
          title="Driver Registration Trends"
          stats={[
        ...
        {/* {["One", "Two", "Three"].map((city) => (
          <StatCard
            formData={formData}
            isEditing={isEditing}
            onInputChange={handleInputChange}
            key={city}
            title={`City ${city} Statistics`}
            stats={[
              { label: "City Name", key: `city${city}` },
              { label: "Total Trips", key: `city${city}TotalTrip` },
              // { label: "Active Drivers", key: `city${city}ActiveDrivers` },
              { label: "Completed Trips", key: `city${city}CompletedTrips` },
            ]}
          />
        ))} */}
      </div>
